...
塞给模型一段话 —— 比如 `"the quick brown fox jumps over"` —— 让它接着往下写。它在*机械上*到底做了什么？从头到尾六步。
...
为什么要换成向量、不直接用整数 ID？因为模型底层只会做线性代数，整数 ID 之间没有任何有用的几何关系 —— token 5 不会因为是连续整数就比 token 100 离 token 6 "更近"。embedding table 给每个 token 在 `d` 维空间里安排了一个*学到的位置*：意思相近的 token 落在附近，没什么关系的 token 离得远。每一行可以理解成模型对那个 token 的"第一印象" —— 还没看到它在句子里的上下文之前，凭空对它的 *feeling*。
...
一个 block 由两个一半组成，每一半都被一个 residual connection 包起来（每一半底下那个小小的 `+` —— 一会儿就解释）：
...
两个一半就是两件主事：一个 **attention** sub-layer，一个 **FFN**（feed-forward network）sub-layer。其它那些零件（LayerNorm、activation、`+`）是小一些的胶水。
...
- **每个 residual `+` 把那一半的 input 重新加回 output。** 所以每一半算的其实是 *delta*，不是 replacement。这就是为什么我们能摞很多 block 而信号不崩。
...
但我们用 LLM 习惯了它一段一段地回复。一个一次只生成一个 token 的模型，怎么生成出一整段？跟你猜的一样：反复跑，把自己的 output 当成下一次的 input 喂回去。
...
6. 采样、追加。序列长度变成 `N+2`。
7. 重复，直到模型采样到一个特殊的 **end-of-sequence** token（它被训练成在认为回答结束时发出这个 token），或者撞到你设定的长度上限。
