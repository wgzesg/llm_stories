...
- **TP 出乎意料地不挡道。** Request batching 没动到它（Article 03），decode/continuous batching 也不会动到。但 PP 和 MoE *会* 跟 TP 有一些有意思的互动 —— 这也是为什么这两篇在地图上排得比较靠前。
- **KV cache 是文章 04 之后的连结组织。** 它一旦在 decode 里登场就不会离开了；它也是长上下文之所以困难的根源。
- **Decode 把瓶颈翻了个面。** 01–03 都假设 prefill，那时候 compute 是主要约束。一旦 decode 进来，weight 读取的 bandwidth 变成卡脖子的那个 —— 这也是后面几乎每个优化（FlashDecoding、GQA、prefill/decode 拆机、speculative decoding）的动机。
